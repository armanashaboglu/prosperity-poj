...
PARAMS = {
    ...
    "KELP": {
        ...
        # Imbalance
        "imbalance_levels": 5, # How many levels deep to calculate imbalance
        "imbalance_factor": 0.2, # How much imbalance affects target price (relative to spread)
        # Volatility (Simple 1-tick move implementation)
        "vol_threshold": 2.0, # Price move threshold to trigger volatility adjustment
        "vol_spread_add": 1, # How many ticks to add to spread when volatile
        "vol_size_divisor": 2, # Divisor for size when volatile (e.g., 2 means half size)
        # Inventory Skew (Target Price)
        "inv_factor": 0.1, # How much inventory affects target price (needs tuning)
        # Spread
        "base_spread": 2,
        "min_spread": 2,
        "max_spread": 8,
        "inventory_warn_level_spread": 40, # Position level to start widening spread
        "inv_spread_add": 1, # How many ticks to add to spread when inventory is high
        # Sizing
        "base_size": 10, # Base size to quote (adjust as needed)
        "inventory_warn_level_size": 30, # Position level to start reducing size
        "inv_size_factor": 0.1, # How much inventory reduces size (proportional, needs tuning)
    },
    "SQUID_INK": {
        # Mirror KELP structure initially - these will be tuned independently
        # EMA
        "ema_alpha": 0.3,
        # Imbalance
        "imbalance_levels": 5,
        ...
        "min_spread": 2,
        "max_spread": 8,
        "inventory_warn_level_spread": 40,
        "inv_spread_add": 1,
        # Sizing
        "base_size": 10,
        "inventory_warn_level_size": 30,
        "inv_size_factor": 0.1,
    }
}
