...
PARAMS = {
    "global": {
        ...
        "soft_limit_factor": 0.55,  # Position pct factor to trigger soft liquidation/skewing (e.g., 0.7 * 50 = 35)
        "skew_offset": 1,          # Price adjustment when position is skewed
        "soft_liq_offset": 2,      # Price adjustment during soft liquidation
        "hard_liq_offset": 1,      # Price adjustment during hard liquidation (0 = trade at fair value)
        "liq_pct": 0.4,            # Percentage of remaining capacity to use during liquidation
        "make_offset": 1,          # Offset for pennying/placing new orders (+/- 1 from best)
    },
    Product.RAINFOREST_RESIN: {
        "fair_value": 10000,
        # Inherits global MM params, might override later if needed
    },
    Product.KELP: {
        # Inherits global MM params
    },
    Product.SQUID_INK: {
        # Inherits global MM params
        "rsi_period": 20,
        "rsi_oversold": 10,
        "rsi_overbought": 90,
        "rsi_skew_factor": 2,     # How many extra price steps to skew based on RSI signal
        "price_history_len": 33 # How many periods of fair value to store for RSI calc
    },
}
