...
#----------------------------
# we receive market_trades dictionary
...
PARAMS = {
    ...
    "KELP": {
        # EMA
        "ema_alpha": 0.3,  # Smoothing factor for EMA (needs tuning)
        # Imbalance
        "imbalance_levels": 8, # How many levels deep to calculate imbalance
        "imbalance_factor": 1.0, # How much imbalance affects target price (relative to spread)
        # Volatility (Simple 1-tick move implementation)
        "vol_threshold": 3.0, # Price move threshold to trigger volatility adjustment
        "vol_spread_add": 0, # How many ticks to add to spread when volatile
        "vol_size_divisor": 1, # Divisor for size when volatile (e.g., 2 means half size)
        # Inventory Skew (Target Price)
        "inv_factor": 0.05, # How much inventory affects target price (needs tuning)
        # Spread
        "base_spread": 3,
        "min_spread": 3,
        "max_spread": 14,
        "inventory_warn_level_spread": 45, # Position level to start widening spread
        "inv_spread_add": 1, # How many ticks to add to spread when inventory is high
        # Sizing
        "base_size": 15, # Base size to quote (adjust as needed)
        "inventory_warn_level_size": 34, # Position level to start reducing size
        "inv_size_factor": 0.25, # How much inventory reduces size (proportional, needs tuning)
    },
    "SQUID_INK": {
        # Parameters only for Z-Score Reversion Strategy
        "rolling_window_z": 10, # Window size for Z-score calculation
        "z_reversion_threshold": 2.9, # Z-score magnitude to trigger trading
        "min_std_dev_z": 0.0268, # Min std dev for Z-score calc
        "z_aggressiveness": 2, # How many ticks to cross BBO (0 = join BBO, 1 = cross by 1)
        "z_trade_size": 9      # Fixed size for reversion trades
        # --- Commented out unused MM Params ---
        # "ema_alpha": 0.6,
        # "imbalance_levels": 4,
        ...
        # "inv_spread_add": 2,
        # "base_size": 17,
        # "inventory_warn_level_size": 27,
        # "inv_size_factor": 0.25,
    }
}
...
# --- Define SquidInkStrategy --- 
class SquidInkStrategy(Strategy): # Inherit directly from base Strategy
    def __init__(self, symbol: str, position_limit: int) -> None:
        super().__init__(symbol, position_limit)
        self.params = PARAMS.get(self.symbol, {}) # Load SQUID_INK specific params
        if not self.params:
             logger.print(f"ERROR: Parameters for {self.symbol} not found in PARAMS.")
             self.params = {} # Use empty dict to avoid errors

        # --- State only for Z-score calculation ---
        # Removed prev_ema, prev_mid
        window_size_z = self.params.get("rolling_window_z", 10)
        self.recent_mids: deque[float] = deque(maxlen=window_size_z)
    ...
    def act(self, state: TradingState) -> None:
        ...
        # --- 1. Read current market state (Best Bid/Ask needed for trade price) ---
        best_bid_price = max(buy_orders.keys()) if buy_orders else None
        best_ask_price = min(sell_orders.keys()) if sell_orders else None

        # --- Calculate Popular Mid Price --- 
        popular_bid_price = None
        max_bid_vol = 0
        if buy_orders:
            sorted_bids = sorted(buy_orders.items(), key=lambda item: (item[1], item[0]), reverse=True) # Sort by vol (desc), then price (desc)
            if sorted_bids: # Check if sorted list is not empty
                popular_bid_price = sorted_bids[0][0]

        popular_ask_price = None
        max_ask_vol = 0 # Track absolute volume
        if sell_orders:
            # Sort by abs(volume) (desc), then price (asc) for tie-breaking
            sorted_asks = sorted(sell_orders.items(), key=lambda item: (abs(item[1]), item[0]), reverse=True)
            if sorted_asks:
                popular_ask_price = sorted_asks[0][0]
        
        current_popular_mid_price = None
        if popular_bid_price is not None and popular_ask_price is not None:
            if popular_ask_price > popular_bid_price: # Ensure valid spread
                current_popular_mid_price = (popular_bid_price + popular_ask_price) / 2
            else:
                 logger.print(f"{self.symbol} -> Popular prices crossed? Bid: {popular_bid_price}, Ask: {popular_ask_price}. Using BBO mid.")
                 # Fallback to BBO mid if popular prices are crossed or invalid
                 if best_bid_price is not None and best_ask_price is not None:
                      current_popular_mid_price = (best_bid_price + best_ask_price) / 2
        elif best_bid_price is not None and best_ask_price is not None:
            # Fallback if only one popular price found, use BBO mid
            logger.print(f"{self.symbol} -> Missing popular price. Using BBO mid.")
            current_popular_mid_price = (best_bid_price + best_ask_price) / 2
        else:
            logger.print(f"{self.symbol} -> Cannot calculate mid price (no BBO or popular). Skipping.")
            return
        
        position = state.position.get(self.symbol, 0)
        logger.print(f"{self.symbol} (Z-Revert) -> PopMid: {current_popular_mid_price:.2f}, Pos: {position}/{self.position_limit}")

        # --- Update recent mids deque (for Z-score) --- 
        self.recent_mids.append(current_popular_mid_price) # Use popular mid price

        # --- 2. Calculate Z-Score --- 
        current_z_score = None
        mean_price_z = None
        min_periods_z = (self.recent_mids.maxlen // 2) # Require at least half the window
        if len(self.recent_mids) >= min_periods_z:
             prices_in_window = list(self.recent_mids)
             mean_price_z = np.mean(prices_in_window)
             std_dev_z = np.std(prices_in_window)
             min_std = self.params.get("min_std_dev_z", 0.01)

             if std_dev_z > min_std:
                 # Calculate Z-score using popular mid price
                 current_z_score = (current_popular_mid_price - mean_price_z) / std_dev_z 
                 logger.print(f"{self.symbol} -> Z-Score (PopMid): {current_z_score:.2f} (Mean: {mean_price_z:.2f}, Std: {std_dev_z:.2f})")
             else:
                 logger.print(f"{self.symbol} -> Std Dev ({std_dev_z:.4f}) too low for Z-Score calc.")
        else:
            logger.print(f"{self.symbol} -> Not enough data ({len(self.recent_mids)}/{min_periods_z}) for Z-Score calc.")

        # --- 3. Determine Trade Signal & Execute --- 
        # (Logic remains the same, but uses the new mean_price_z derived from popular mids)
        if current_z_score is not None and mean_price_z is not None: 
            z_thresh = self.params.get("z_reversion_threshold", 1.5)
            trade_size = self.params.get("z_trade_size", 5)
            aggressiveness = self.params.get("z_aggressiveness", 0)
            to_buy_capacity = self.position_limit - position
            to_sell_capacity = self.position_limit + position

            # --- Signal: Sell on High Z-Score (Price >> Mean) ---
            if current_z_score > z_thresh and to_sell_capacity > 0:
                 if best_bid_price is not None and best_bid_price > mean_price_z:
                     sell_price = best_bid_price - aggressiveness
                     size_to_sell = min(trade_size, to_sell_capacity)
                     if size_to_sell > 0:
                          logger.print(f"{self.symbol} -> Z-REVERT SELL SIGNAL (Z: {current_z_score:.2f} > {z_thresh}) @ {sell_price}")
                          self.sell(sell_price, size_to_sell)
                 else:
                     logger.print(f"{self.symbol} -> Z-REVERT SELL SIGNAL but no viable bids (Bid: {best_bid_price}, Mean: {mean_price_z:.2f}).")

            # --- Signal: Buy on Low Z-Score (Price << Mean) ---
            elif current_z_score < -z_thresh and to_buy_capacity > 0:
                 if best_ask_price is not None and best_ask_price < mean_price_z:
                     buy_price = best_ask_price + aggressiveness
                     size_to_buy = min(trade_size, to_buy_capacity)
                     if size_to_buy > 0:
                          logger.print(f"{self.symbol} -> Z-REVERT BUY SIGNAL (Z: {current_z_score:.2f} < {-z_thresh}) @ {buy_price}")
                          self.buy(buy_price, size_to_buy)
                 else:
                      logger.print(f"{self.symbol} -> Z-REVERT BUY SIGNAL but no viable asks (Ask: {best_ask_price}, Mean: {mean_price_z:.2f}).")
            

    def save(self) -> dict:
        # Save only the state needed for this strategy
        return {
            "recent_mids": list(self.recent_mids) # Save deque as list
        }

    def load(self, data: dict) -> None:
        # Load only the state needed for this strategy
        loaded_mids = data.get("recent_mids", [])
        window_size_z = self.params.get("rolling_window_z", 10)
        self.recent_mids = deque(loaded_mids, maxlen=window_size_z)
        if len(loaded_mids) > window_size_z:
            self.recent_mids = deque(loaded_mids[-window_size_z:], maxlen=window_size_z)
...
class Trader:
    def __init__(self):
        ...
        self.strategies = {
            ...
            # --- Use the new SquidInkStrategy --- 
            "SQUID_INK": SquidInkStrategy("SQUID_INK", self.position_limits["SQUID_INK"])
        }
