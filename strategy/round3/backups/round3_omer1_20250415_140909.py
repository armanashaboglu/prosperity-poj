...
PARAMS = {
    "SQUID_INK": { 
        ...
        "rsi_overbought": 56, 
        "rsi_oversold": 39, 
        "price_offset": 1,  # New parameter for SQUID_INK to offset prices
    },
    "RAINFOREST_RESIN": { 
        "fair_value": 10000,
    },
    # --- Parameters for the new Deviation Strategy ---
    ...
    # --- NEW: Parameters for Voucher Slope Reversion ---
    "VOUCHER_SLOPE_REVERSION": {
        "rolling_window": 410,      # Lookback window for slope SMA/STD
        "std_dev_multiplier": 1.8,  # k value for bands (multiplier for std dev)
        "trade_size": 20,           # Base quantity to trade per signal
        "min_history": 50,          # Minimum slope data points before trading
        "exit_target": "mean",      # "mean" or could be a fixed z-score like 0.0
        "recalc_stats_on_load": True, # Recalculate MA/STD/Bands when loading state
        "liquidity_capture_pct": 1.0, # Max % of available liquidity to capture per order
        "min_trade_size": 5,        # Minimum trade size to consider worth executing
        "allowed_pairs": [          # Only trade these pairs to avoid conflicts 
            "9500-9750",            # Format: "low_strike-high_strike"
            "10000-10250"           # Skip 9750-10000 to avoid conflicts
        ]
    },
    # --- NEW: Parameters for Volcanic Rock RSI ---
    Product.VOLCANIC_ROCK: {
        "rsi_window": 106,      # Placeholder - OPTIMIZE LATER
        "rsi_overbought": 52,  # Placeholder - OPTIMIZE LATER
        "rsi_oversold": 41,    # Placeholder - OPTIMIZE LATER
        "price_offset": 0,     # No offset for VOLCANIC_ROCK
    },
    # --- NEW: Parameters for 10500 Voucher Strategy ---
    "VOUCHER_10500_STRATEGY": {
        "min_rock_price": 10200,     # Only trade when volcanic rock price is above this
        "rsi_window": 106,           # Same as volcanic rock 
        "rsi_overbought": 52,        # Adjusted for voucher
        "rsi_oversold": 41,          # Adjusted for voucher
        "max_position_pct": 1.0,     # Use 100% of position limit
    }
}
